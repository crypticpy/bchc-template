...
| Front-end code & perf | no file >500 lines / no duplicated logic; every failure path user-visible; no string `innerHTML` for content; Lighthouse ≥95 perf & a11y with subset fonts; unit tests on pure logic | lints clean, ~90 | budgets in CI; every pure function tested; modules ≤ ~250 lines |
